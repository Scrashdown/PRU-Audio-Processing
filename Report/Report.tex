...
\caption{The BBB with the Octopus Board, ready to be used!}
\end{figure}
...
The remaining registers are used for storing channel private data. In order to figure out how many registers we need for this, assuming one register is wide enough to hold the expected values (see the formula above to check the required bit width), we can use the following formula \texttt{(M = 1)} :
