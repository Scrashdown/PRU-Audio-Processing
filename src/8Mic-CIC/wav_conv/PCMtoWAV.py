...
raw_u16 = np.fromfile(in_file, dtype=np.uint16, count=-1, sep='')[offset:]
...
temp = np.subtract(first_channel, (first_channel.max() + first_channel.min()) / 2)
...
wf.write(out_file, 39000, norm_f32)
