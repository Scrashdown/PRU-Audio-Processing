...
void processing(FILE * output, const uint8_t * host_buffer, size_t sample_count, volatile uint32_t * PRUmem) {
    for (size_t counter = 0; counter < sample_count; ++counter) {
        ...
        // Read 2 * 4 bytes from PRUmem, because each sample holds on 4 bytes
        memcpy((void *) &host_buffer[2 * 4 * counter], (const void *) PRUmem, 2 * 4);
    }

    // Write the result to the file
    size_t written = fwrite(host_buffer, 2 * 4, sample_count, output);
...
int main(int argc, char ** argv) {
    ...
    FILE * output = fopen("../output/32bits_2chan.pcm", "wb");
    if (output == NULL) {
